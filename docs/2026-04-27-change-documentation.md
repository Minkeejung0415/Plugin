...
- **Device editor layout (Red Pitaya):** Filter, analog in/out, sample rate, and RECORD are grouped in the **left column** (same x positions as the sample-rate and RECORD references) so the channel canvas does not overlap those controls during acquisition. The temporary `toFront()` workaround after `beginAnimation()` was removed.
